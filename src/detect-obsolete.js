...
fs.readFile('floorp_en.ftl', 'utf-8', (err, data1) => {
  ...
  fs.readFile('floorp.ftl', 'utf-8', (err, data2) => {
    ...
    fs.readFile('exception_keys.txt', 'utf-8', (err, data3) => {
      ...
      // Check if the keys in the second file are not present in the first file, except for the exception keys
      keys2.forEach(key => {
        if (!exceptionKeys.has(key) && !keys1.has(key)) {
          console.log(`Key '${key}' is obsolete.`);
        }
      });
    });
  });
